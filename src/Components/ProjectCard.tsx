...
import FullProjectModel from "./FullProjectModal";
   ...
   const badge=useMatches({
    ...
    return (<div className="w-[32%] lg-mx:w-[46%] md-mx:w-[48%] sm-mx:w-[90%] xs-mx:w-full" data-aos="fade-up" data-aos-duration="800">
      ...
      <Group  mb="xs" className="!gap-2" >
        {props.technologies.map((tech: string , index: number)=> index<3 && <Badge key={index} size="lg" color="#64ffda" variant="light">{tech}</Badge>)}
      </Group>
     ...
     <FullProjectModel opened={opened} close={close}  title={props.title} description={props.description} image={props.image} link={props.link} github={props.github} live={props.live} technologies={props.technologies} /> 
      </div>
    )
 }
